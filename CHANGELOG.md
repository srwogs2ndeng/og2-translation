...
### Changed
- **Generic battle quotes no longer give the opponent a gender.** Most battle lines play
  against whoever the pilot happens to be fighting, and the Japanese is written that way:
  こいつ, 奴, あいつ, or no pronoun at all. The English had supplied "he" and "that guy"
  anyway. A reviewer caught Sleigh saying "he had this much power in him" after Ibis and
  Tsugumi hit her. 337 lines now use they/them/their, or rephrase ("that one").

  Each line was read individually, not pattern-replaced. A gendered word stays where the
  Japanese itself is gendered (小娘, とっつぁん, 漢), for honorifics and titles (Lady Tytti,
  His Highness), for a ship's "her", for "man" meaning humanity, and where the pronoun
  really does mean a named person. A name in the line is not enough on its own: "Hugo,
  leave him to me" means the enemy, not Hugo, and is now "leave them to me". The rule is
  in `docs/TRANSLATION-STYLE.md`.

### Fixed
- **Ing's unit was called "Alteisen".** アルブレード is "R-Blade Custom" in the game's own
  unit data, but the battle lines wrote it as Alblade, Aull Blade, and - in Ing's two lines -
  Alteisen. All 13 now say R-Blade.
- **The NEXT prompt rendered as `< /e6t>`.** The label is `＜NEXT  ＞`, with the game's own
  full-width brackets. The English used ASCII `<` `>`, and a `<` followed by letters is
  read as a control tag, which swallowed the word. `<Add Weapon>` had the same fault.
  Both restored.
- **Chapter 45 called Egretta Uno "the Egretta"** in five lines, as if he were a ship,
  and several other lines in the scene read stiffly. 13 lines corrected.
- **Chapter 46: "Cross Sorcer" is Cross Saucer** in the weapon list, matching what the
  battle quotes already said.

### Known
- Three command-menu strings, `<Repair>`, `<Resupply>` and `<Recover>`, use the same
  ASCII brackets. Their slots are 12 bytes, and two of the words do not fit once the
  brackets are full-width, so they stay as they are until a shorter wording is chosen.
