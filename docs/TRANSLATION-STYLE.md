...
- **Generic battle quotes do not know who they are fighting, so they must not give the
  opponent a gender.** Most battle lines play against whoever the pilot happens to be
  facing, and the Japanese is written that way: こいつ, 奴, あいつ, 相手, 敵, or no
  pronoun at all. Use they/them/their, or rephrase ("this pilot", "that kind of power"),
  never he/she and never "that guy". Keep a gendered word only when the Japanese itself
  is gendered (彼, 彼女, おっさん, 爺さん, 小僧) or the line names the person it is
  about. A name in the line is not proof on its own: in "he's reacting to Cybuster's
  speed" the name is the speaker's own machine and the pronoun is still the unknown enemy.
