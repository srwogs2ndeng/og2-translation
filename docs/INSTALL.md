...
> **Platform:** developed and tested end to end on **Windows**. On Linux the build is
> verified to produce byte-identical archives, but the EBOOT step, the GUI and booting the
> result are untested; macOS is untested entirely. Use `python3` throughout on those.
