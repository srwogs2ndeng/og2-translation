...
result played. On **Linux** the build is verified — run against a pristine dump on a
case-sensitive filesystem, all five patched archives come out with the same sha256 as the
Windows build, so the data you get is identical — but the optional letter-spacing step,
the GUI window itself, and actually booting the result have not been tested there.
**macOS is untested.** It is all pure Python and there is no reason to expect trouble,
but "no reason to expect trouble" is not the same as tested, and it would be dishonest to
present it as such.

## Known limitations

- Tested end to end on Windows only; see *Platform support* above.
