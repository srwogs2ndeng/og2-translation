...
**Windows is the only platform this is tested end to end on** — built, deployed, and the
result played in RPCS3. That is where it is developed, so that is what "it works" means.

**Linux: the build is verified, the rest is not.** The shipped zip has been run on a
case-sensitive filesystem against a pristine dump, and all five patched archives came out
with the same sha256 as the Windows build, so the patch data a Linux user gets is the same
bytes. What has *not* been tested on Linux: the optional `--eboot-elf` step, the GUI window
itself (only its dependency check), and booting the result in RPCS3.

**macOS is untested.** It is the same pure Python and should behave like Linux, but nobody
has run it.

If you hit something on a platform other than Windows, it is worth reporting — that is the
...
- **Only tested end to end on Windows.** The build itself is verified on Linux (identical
  output archives), but the EBOOT step, the GUI window and booting the result are not.
  macOS is untested. See *Platform support* above.
