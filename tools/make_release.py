...
PLATFORMS. Windows is where this is developed, built and played. Linux and the Steam
Deck work: the build is verified here (the same five archives, with the same sha256 as
the Windows build, on a case-sensitive filesystem) and several people have reported the
result booting on a Steam Deck with the English rendering correctly - user reports, not
reproduced here. Two pieces are still Windows-only tested: the GUI window, and the
optional EBOOT step. macOS is untested entirely.
