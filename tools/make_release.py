...
TESTED END TO END ON WINDOWS ONLY. That is where it is developed, built and played.
On Linux the build is verified - the same five patched archives come out with the same
sha256 as the Windows build, on a case-sensitive filesystem - but the optional EBOOT
step, the GUI window itself, and booting the result have NOT been tested there. macOS
is untested entirely. It is all pure Python and expected to work; "expected" is the
honest word. Reports from non-Windows platforms are useful.
...
WHAT YOU NEED, none of which is in this zip:
  ...
  4. Python 3.10 or newer, then:  pip install cryptography
     (capstone as well, but ONLY if you do the optional step 3 below)

     On Debian/Ubuntu the stock python3 has neither tkinter nor ensurepip, which the GUI
     and venv respectively need:  sudo apt install python3-tk python3-venv
     On Arch/SteamOS:             sudo pacman -S tk

INSTALL
  1. Configure RPCS3 for this game:
         python3 tools/setup_rpcs3.py "<your RPCS3 folder>"

  2. Patch your copy of the game. Either use the GUI -

         Windows      double-click  Install (GUI).bat
         Linux/macOS  run           ./install-gui.sh

     or run:

         python3 apply.py "<...>/BLJS10133/PS3_GAME/USRDIR" --gd "<...>/dev_hdd0/game/BLJS10133"
...
def modes():
    """rel -> unix mode, taken from git's own index (100755 vs 100644).

    zipfile.writestr with a plain name stamps 0600 and no execute bit, so a shell script
    comes out of the zip non-executable and `./install-gui.sh` fails with Permission
    denied. Windows does not care; Linux and macOS users are the ones who get the zip's
    permissions applied by unzip."""
    r = subprocess.run(["git", "ls-files", "-s"], cwd=REPO, capture_output=True, text=True)
    out = {}
    for line in r.stdout.splitlines():
        parts = line.split("\t", 1)
        if len(parts) == 2:
            out[parts[1]] = 0o755 if parts[0].split()[0] == "100755" else 0o644
    return out
...
def main():
    ...
    def put(arc, data, unix_mode=0o644):
        zi = zipfile.ZipInfo(arc, date_time=time.localtime(time.time())[:6])
        zi.compress_type = zipfile.ZIP_DEFLATED
        # create_system MUST say UNIX (3). Built on Windows, ZipInfo defaults it to 0
        # (FAT), and then unzip(1) reads external_attr as DOS attribute bits and throws
        # the unix mode away - the file lands 0644 and the shell script is not
        # executable. The mode below is only meaningful together with this line.
        zi.create_system = 3
        ...
        for rel in files:
            ...
            if rel.endswith((".json", ".md", ".py", ".txt", ".yml")):
                try:
                    ...
                    pass
            put(TOP + "/" + rel, data, mode.get(rel, 0o644))
        if locked and "build/jp_vault.enc" not in files:
            put(TOP + "/build/jp_vault.enc", open(vault, "rb").read())   # present but untracked
        put(TOP + "/INSTALL.txt", INSTRUCTIONS)
