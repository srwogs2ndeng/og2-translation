...
# grep -c PRINTS 0 and EXITS 1 when there is no match, so an "|| echo 0" fallback
# appended a SECOND zero and every later integer test died on a two-line value.
# Pipe through head instead: the count was already right, only the exit status lied.
stops()   { [ -f "$LOG" ] || { echo 0; return; }; grep -a -c "Stopping emulator" "$LOG" 2>/dev/null | head -1; }
