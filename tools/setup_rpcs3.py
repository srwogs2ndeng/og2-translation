...
def find_config_dir(rpcs3_dir):
    ...
    if os.name == "nt":
        ...
        candidates.append(os.path.join(home, ".config", "rpcs3", "custom_configs"))
        # Flatpak, which is how the Steam Deck installs RPCS3: the sandbox redirects
        # XDG_CONFIG_HOME into ~/.var/app/<app-id>/config, so rpcs3's own config folder
        # sits one level further in than a native install.
        candidates.append(os.path.join(home, ".var", "app", "net.rpcs3.RPCS3",
                                       "config", "rpcs3", "custom_configs"))
    for c in candidates:
