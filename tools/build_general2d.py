...
-> wtd_positions (per-element X nudges, for records that carry no font size)
...
def build():
    ...
    # positions come last: some labels are mispositioned rather than oversized, and those
    # records carry no font size for wtd_sizes to touch (see wtd_positions.py)
    run(T("wtd_positions.py"), WTD_EN, WTD_EN)
    ...
    os.makedirs(os.path.dirname(PSARC_OUT), exist_ok=True)   # generated-only dir; git ships no such thing
