...
"""
import sys, os, json, re, shutil, subprocess, glob, datetime
...
MISSING_SRC = """!! no extracted file for the worksheet %s
!!   expected: %s
!! The worksheet names a file that is not in your extracted archive. Check the
!! case of the path - it matters on Linux and macOS, and not on Windows."""

APPLY_FAILED = """!! failed applying one worksheet; nothing was packed or deployed
!!   file:      %s
!!   worksheet: %s
!!   error:     %s: %s
!! Please report that path. Deleting the worksheet does let the run finish, but it
!! silently leaves those lines untranslated."""
...
def cmd_apply(name):
    ...
    for sh in sheets:
        ...
        if not os.path.isfile(jp_file):
            raise SystemExit(MISSING_SRC % (rel, jp_file))
        ...
        try:
            ...
            # One bad worksheet aborted the whole container with a traceback naming no
            # file, leaving the user to guess which of 300-odd it was. A 2026-09-10
            # report had someone deleting worksheets to find out, which does let the run
            # finish but silently drops those lines back to Japanese.
            raise SystemExit(APPLY_FAILED % (rel, sh, type(e).__name__, e))
        n += 1
...
def cmd_pack(name):
    ...
    # build/out holds only generated archives, so git cannot track it (it tracks files, not
    # directories) and a fresh clone or release zip does not have it. This machine has had
    # one since the first build, which is why the miss survived every release: the very
    # first pack of a fresh install died here on FileNotFoundError.
    os.makedirs(os.path.dirname(out_psarc), exist_ok=True)
...
ROLLBACK_KEEP = 3          # per container; each one is a full copy of the live archive
_TS = re.compile(r"\d{8}-\d{6}")


def prune_rollbacks(name, keep=ROLLBACK_KEEP):
    """Drop all but the newest `keep` backups for one container.

    Every deploy copies the whole live archive aside, and Battle is 1.6 GB a time, so an
    unbounded folder is a slow disk leak: it had reached 61 GB across 182 files before
    anyone noticed, 90 of them Logic.

    Ordering is by the TIMESTAMP IN THE FILENAME, never mtime. The backup is made with
    shutil.copy2, which preserves the SOURCE file's mtime, so these files carry the game
    archives' dates - some of them read 2012 - and sorting by mtime would delete the
    wrong ones. Files whose names carry no timestamp are left alone entirely."""
    bdir = os.path.join(REPO, "build", "rollbacks", name)
    if not os.path.isdir(bdir):
        return
    fs = [f for f in os.listdir(bdir)
          if os.path.isfile(os.path.join(bdir, f)) and _TS.search(f)]
    fs.sort(key=lambda f: _TS.search(f).group(0), reverse=True)
    freed = 0
    for f in fs[keep:]:
        ...
        os.remove(p)
    if freed:
        print("  pruned %d old rollback(s) for %s, freed %.2f GB"
              % (len(fs) - keep, name, freed / 1e9))
