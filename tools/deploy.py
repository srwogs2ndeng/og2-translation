...
Commands:
  ...
  deploy  <Archive>          backup live SDAT, copy new -> game folder, MIRROR into every GD
                             tree (no install screen; add --wipe to force a reinstall instead)
...
def cmd_deploy(name):
    ...
    if "--wipe" in sys.argv:
        _wipe_gd()
        print("  GD wiped -> next boot reinstalls from folder (ONLY works on RPCS3 builds whose "
              "cellGameDataCheck reports a missing dir; the current retail build does NOT)")
    else:
...
def _sync_gd(name, sdat_path):
    """Mirror the deployed archive into EVERY game-data tree, creating a complete GD from the
    template if it is missing. 2026-09-01: the 'retail' RPCS3 is now v0.0.41-19638, whose
    cellGameDataCheck CREATES /dev_hdd0/game/BLJS10133/USRDIR during the check and reports it
    as existing -> this game then sees 'existing but empty' data and shows "Game data is
    corrupted" instead of installing. A fresh install is therefore IMPOSSIBLE on that build,
    so we no longer wipe: we lay the GD down ourselves exactly as the game's installer would
    (PARAM.SFO + ICON0.PNG as the installer wrote them, kept in build/gd_template, plus the
    five USRDIR/PSARC/*.sdat mirrored from the disc folder). Works on both emulator builds and
    skips the install screen entirely."""
